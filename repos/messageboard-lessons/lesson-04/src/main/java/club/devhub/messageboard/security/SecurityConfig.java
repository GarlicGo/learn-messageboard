package club.devhub.messageboard.security;


import club.devhub.messageboard.filter.TokenFilter;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, AuthenticationFailedHandler authenticationFailedHandler, TokenFilter tokenFilter) throws Exception {
        ...
        http.authorizeRequests().antMatchers(
                "/api/msgboard/v1/users/register",
                "/api/msgboard/v1/users/login",
                "/api/msgboard/v1/admins/login",
                "/actuator/**"
        ).permitAll()
